...
class Generator extends \yii\gii\Generator
{
    ...
    public $componentNs = 'common\components';
    ...
    public function getDescription()
    {
        return 'This generator generates a controller and views that implement CRUD (Create, Read, Update, Delete)
            operations for the specified data model with use of handler/actions from component.';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['controllerClass', 'modelClass', 'searchModelClass', 'baseControllerClass', 'componentNs'], 'filter', 'filter' => 'trim'],
            [['modelClass', 'controllerClass', 'baseControllerClass', 'indexWidgetType', 'componentNs'], 'required'],
            [['searchModelClass'], 'compare', 'compareAttribute' => 'modelClass', 'operator' => '!==', 'message' => 'Search Model Class must not be equal to Model Class.'],
            [['modelClass', 'controllerClass', 'baseControllerClass', 'searchModelClass', 'componentNs'], 'match', 'pattern' => '/^[\w\\\\]*$/', 'message' => 'Only word characters and backslashes are allowed.'],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            ...
            'componentNs' => 'Component namespace',
        ]);
    }
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            'componentNs' => 'Type component namespace. Example: common\components\exchange',
        ]);
    }
    ...
    {
        return array_merge(parent::stickyAttributes(), ['baseControllerClass', 'indexWidgetType', 'componentNs']);
    }
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field(\$model_form, '$attribute')->passwordInput()";
            }

            return "\$form->field(\$model_form, '$attribute')";
        }
        $column = $tableSchema->columns[$attribute];
        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model_form, '$attribute')->checkbox()";
        }

        if ($column->type === 'text') {
            return "\$form->field(\$model_form, '$attribute')->textarea(['rows' => 6])";
        }
        ...
        if (is_array($column->enumValues) && count($column->enumValues) > 0) {
            ...
            return "\$form->field(\$model_form, '$attribute')->dropDownList("
                . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)).", ['prompt' => ''])";
        }

        if ($column->phpType !== 'string' || $column->size === null) {
            return "\$form->field(\$model_form, '$attribute')->$input()";
        }

        return "\$form->field(\$model_form, '$attribute')->$input(['maxlength' => true])";
    }
    ...
    /**
     * Generates action parameters with type (int) for $id param
     * @return string
     */
    public function generateActionParamsWithType()
    {
        /* @var $class ActiveRecord */
        $class = $this->modelClass;
        $pks = $class::primaryKey();
    ...
    {
        if (mb_strpos($this->modelClass, 'common\components\\') === 0){
            $right = mb_substr($this->modelClass, mb_strlen('common\components\\'));
            $component_name = mb_substr($right, 0, mb_strpos($right, '\\'));
            return 'common\components\\' . $component_name . '\\forms\\';
        }
        return $this->modelClass . '\\forms\\';
    }
}
